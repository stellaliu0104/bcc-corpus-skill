...
5. **解读**（见下方输出规范）。
6. **结果文件**：凡命中数较多（>30 条）或用户要看全部结果时，加 `--export xlsx`（或 `html`）重跑一次，把生成的文件**绝对路径**告诉用户（默认在桌面 `bcc-results/` 文件夹），并说明可用 Excel 打开筛选排序；`html` 版双击打开可搜索/点表头排序。用户说"导出 Excel/导出表格/给我数据库"均指此功能。
