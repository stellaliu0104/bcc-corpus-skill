...
5. **确认结果页**：读取输出的 `results_pages`，向用户说明已打开的页面路径与展示条数。列表默认每页 50 条，**只显示查询目标所在句及前后共约 50 字**，查询目标以黄色高亮；可筛选、排序、翻页。用户点击“查看完整段落”后才显示完整语境；因发布语料已按句切分，完整段落定义为命中句前后连续 3 句。页面可导出 CSV（Excel 可打开）或独立 HTML。若 `truncated:true`，说明命中超过 5000 条，页面只展示前 5000 条；若某条标为“未定位”，不得把它说成已有确定出处。
