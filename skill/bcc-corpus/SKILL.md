...
5. **确认结果页**：读取输出的 `results_pages`，向用户说明已打开的页面路径与展示条数。结果页包含：关键词左右文（KWIC）、所在完整句、语料文件出处和行号；默认每页 50 条，可筛选、排序、翻页，并可在页面内点击导出 CSV（Excel 可打开）或独立 HTML。若 `truncated:true`，说明命中超过 5000 条，页面只展示前 5000 条；若某条标为“未定位”，不得把它说成已有确定出处。
