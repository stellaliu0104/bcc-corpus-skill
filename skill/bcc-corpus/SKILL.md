...
5. **确认结果页**：读取输出的 `results_pages`，向用户说明已打开的页面路径与展示条数。列表默认每页 50 条，**只显示查询目标所在句及前后共约 50 字**，紧凑单行展示、查询目标以黄色高亮；可筛选、排序、翻页。用户点击“查看完整段落”后才显示完整语境：发布语料已按句切分、原始段落边界不再保留，因此展示以命中词为中心前后各最多 **200 个连续字符**，并按连续句子换行显示。页面可导出 CSV（Excel 可打开）或独立 HTML。`未定位` 不代表未命中，而是引擎没有提供来源且本地 KWIC 回查无法可靠唯一确定源文件；不得把它说成已有确定出处。若 `truncated:true`，说明命中超过 5000 条，页面只展示前 5000 条。
